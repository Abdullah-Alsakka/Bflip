# -*- coding: utf-8 -*-
"""
Created on Thu Jul 27 21:51:29 2023
...
for plane in planes:
    for sp in species:
        for var in variables:
            os.mkdir(f'D:/abd__/Downloads D/Bflip/Plots/{var}_{plane}_{sp}_CC')
            ...
            for i in range(len(file_list)):
                ...
                plt.pcolormesh(X, Y, rho, shading='auto', cmap='Greys')
                ...
                plt.quiver(X, Y, normalized_x, normalized_y, magnitude, cmap='cool'
                           , scale=20, lw = 0.1, width = 0.006)
                #plt.xlabel('X')
                #plt.ylabel('Y')
                plt.title(f'{var} Field in the {plane} Plane for {molecules[species.index(sp)]}')
                plt.colorbar(label='Magnitude')
                
                plt.savefig(f'D:/abd__/Downloads D/Bflip/Plots/{var}_{plane}_{sp}_CC/plot-{i}.png')
                plt.show()
                #plt.close()